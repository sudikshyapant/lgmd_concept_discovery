...
applies the paper's three filters (filler removal, class-name overlap, CLIP-text
near-duplicate dedup) and keeps exactly `r = 25`. Editing the file invalidates the
concept cache automatically (its content hash feeds `cache_name`). Add a new class by
adding a key with ≥ `r` candidates and setting `CONFIG["class_name"]` to match.
